...
class RenderEncoderApp : public test::App
{
private:
	ngfxu::PresentLayer		layer_;
	ngfxu::CommandQueue		queue_;
	ngfxu::RenderPipeline	render_pipeline_;
public:
	RenderEncoderApp(k3d::String const& appName) : test::App(appName, 1600, 1080)
	{
	}
	...
	virtual bool OnInit() override
	{
		...
		render_pipeline_ = device_.newRenderPipeline(ngfx::RenderPipelineDesc());
	...
	virtual void OnProcess(k3d::Message& message) override
	{
		...
		renderCmd.setPipeline(render_pipeline_);

	}
};


K3D_APP_MAIN(RenderEncoderApp);